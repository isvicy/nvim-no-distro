...
基于 Treesitter 的 Markdown 标题折叠功能，能够精准识别代码块，避免将代码块内的
`#` 注释误判为标题。
...
使用 Treesitter 预先解析文档，缓存所有 `fenced_code_block` 节点的行范围，在
`foldexpr` 中通过简单的范围查找判断当前行是否在代码块内。
...
`foldexpr` 会被 Neovim 频繁调用（每行都会调用），如果每次都执行 Treesitter
查询会导致严重的性能问题。
